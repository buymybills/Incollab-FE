...
const ApplicationDetailScreen = ({ onBack, showApplyButton, selectedCampaignId }: ApplicationDetailScreenProps) => {
    ...
    const {mutate: applyApplication, isPending: loadingApplyApplication} = useMutationApi({
    ...
    <div className={`h-full flex flex-col bg-white ${showApplyButton ? 'pb-24' : 'pb-0'}`}>
      ...
      <div className='fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200'>
        <button
          ...
          disabled={loadingApplyApplication}
        ...
        >
          {loadingApplyApplication ? 'Applying...' : 'Click to Apply'}
        </button>
      </div>
    </div>
  )
